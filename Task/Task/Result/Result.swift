...
public enum Result<Value, Error>: ResultProtocol, CustomStringConvertible, CustomDebugStringConvertible {
	case success(Value)
	case failure(Error)
	...
	public init(attempt f: () throws -> Value) {
		do {
			...
			self = .failure(error)
        } catch where Error.self == AnyError.self {
            self = .failure(AnyError(error) as! Error)
        } catch {
...
extension Result where Error: Equatable {
